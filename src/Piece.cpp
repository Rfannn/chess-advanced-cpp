...
bool Pawn::isValidMove(int fr, int fc, int tr, int tc, const Board& b) const {
    ...
    // forward (only to empty square)
    if (tc == fc && dr == dir && b.at(tr, tc) == nullptr) return true;

    //double(from start square +the next two squares gotta be white)
    if (fr == startRow && tc == fc && dr == 2 * dir && b.at(tr, tc) == nullptr
        && b.at(fr + dir, fc) == nullptr)
        return true;

    return false;
}

bool Knight::isValidMove(int fr, int fc, int tr, int tc, const Board& b) const {
    int dr = abs(tr - fr);
    int dc = abs(tc - fc);
...
bool King::isValidMove(int fr, int fc, int tr, int tc, const Board& b) const {
    int dr = abs(tr - fr);
    int dc = abs(tc - fc);
